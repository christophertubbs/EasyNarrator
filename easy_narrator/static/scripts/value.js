...
export class EventValue {
    static create(initialValue, ...onUpdate) {
        const newValue = new this(initialValue, ...onUpdate);
        return [newValue.get, newValue.set, newValue.onUpdate];
    }

    static descriptor(initialValue, ...onUpdate) {
        const newValue = new this(initialValue, ...onUpdate);
        return {
            get: newValue.get,
            set: newValue.set,
...
export class ListValue {
    ...
    /**
     *
     * @type {((event: ListValueEvent<T>) => any)[]}
     */
    #handlers = []
    ...
    /**
     *
     * @param initialValue {T[]|undefined}
     * @param handlers{(event: ListValueEvent<T>) => any}
     */
...
export class NumericValue extends EventValue {
    increment = (amount) => {
        if (amount === null || typeof amount === 'undefined') {
            amount = 1;
        }

        const newValue = this.get() + amount;
...
narrator.EventValue = EventValue;
narrator.BooleanValue = BooleanValue;
narrator.ListValue = ListValue;
narrator.ListValueAction = ListValueAction;
narrator.ListValueEvent = ListValueEvent;