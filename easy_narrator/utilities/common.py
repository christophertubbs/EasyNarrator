...
@typing.overload
def now(as_string: typing.Literal[True]) -> str:
    pass


@typing.overload
def now(as_string: typing.Optional[typing.Literal[False]] = False) -> datetime:
    pass


def now(as_string: bool = False) -> typing.Union[datetime, str]:
    this_instant: datetime = datetime.now().astimezone()
    if as_string:
        return this_instant.strftime(DATETIME_FORMAT)
    return this_instant
...
def get_html_response_from_text(
    ...
    """
    if context:
        _LOGGER.warning("Context management for HTML responses has not been implemented yet")
